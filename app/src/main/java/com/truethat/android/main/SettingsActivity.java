...
import com.truethat.android.utils.DateUtils;
import com.truethat.android.utils.FileUtils;
import com.truethat.android.utils.ImageUtils;
...
public class SettingsActivity extends AppCompatActivity {
  ...
  @Override protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    mSwitchNotificationsShowPreviews =
        (SwitchCompat) findViewById(R.id.switch_notifications_show_previews);

    mSwitchNotificationsDoNotDisturb =
        (SwitchCompat) findViewById(R.id.switch_notifications_do_not_disturb);
    mLinearLayoutDoNotDisturb = (LinearLayout) findViewById(R.id.linear_layout_do_not_disturb);
    mLinearLayoutNotificationsDoNotDisturbFrom =
        (LinearLayout) findViewById(R.id.linear_layout_notifications_do_not_disturb_from);
    mLinearLayoutNotificationsDoNotDisturbTo =
        (LinearLayout) findViewById(R.id.linear_layout_notifications_do_not_disturb_to);
    mTextViewNotificationsDoNotDisturbFrom =
        (TextView) findViewById(R.id.text_view_notifications_do_not_disturb_from);
    ...
    if (profileUrl.length() > 0) {
      ImageUtils.displayRoundImageFromUrlWithoutCache(SettingsActivity.this, profileUrl,
          mImageViewProfile);
    }
    ...
    mSwitchNotificationsShowPreviews.setChecked(
        PreferenceUtils.getNotificationsShowPreviews(SettingsActivity.this));
    ...
    mSwitchNotifications.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
      @Override public void onCheckedChanged(CompoundButton buttonView, final boolean isChecked) {
        if (isChecked) {
          PushUtils.registerPushTokenForCurrentUser(SettingsActivity.this,
          ...
          PushUtils.unregisterPushTokenForCurrentUser(SettingsActivity.this,
              new SendBird.UnregisterPushTokenHandler() {
                @Override public void onUnregistered(SendBirdException e) {
                  if (e != null) {
                    mSwitchNotifications.setChecked(!isChecked);
                    checkNotifications(!isChecked);
                    return;
    ...
    mSwitchNotificationsShowPreviews.setOnCheckedChangeListener(
    ...
    SendBird.getDoNotDisturb(new SendBird.GetDoNotDisturbHandler() {
      @Override
      public void onResult(boolean isDoNotDisturbOn, int startHour, int startMin, int endHour,
          int endMin, String timezone, SendBirdException e) {
        mCalendar.clear();
        ...
        PreferenceUtils.setNotificationsDoNotDisturbFrom(SettingsActivity.this,
        ...
        PreferenceUtils.setNotificationsDoNotDisturbTo(SettingsActivity.this,
  ...
  private void saveDoNotDisturb(final boolean doNotDisturb) {
    String doNotDisturbFrom =
        PreferenceUtils.getNotificationsDoNotDisturbFrom(SettingsActivity.this);
    ...
    if (doNotDisturbFrom.length() > 0 && doNotDisturbTo.length() > 0) {
      ...
      SendBird.setDoNotDisturb(doNotDisturb, startHour, startMin, endHour, endMin,
          TimeZone.getDefault().getID(), new SendBird.SetDoNotDisturbHandler() {
  ...
  private void setDoNotDisturbTime(final boolean from, final TextView textView) {
    ...
    new TimePickerDialog(SettingsActivity.this, new TimePickerDialog.OnTimeSetListener() {
      @Override public void onTimeSet(TimePicker timePicker, int hour, int min) {
        mCalendar.clear();
        ...
        if (from) {
          if (!String.valueOf(millis)
              .equals(PreferenceUtils.getNotificationsDoNotDisturbFrom(SettingsActivity.this))) {
            PreferenceUtils.setNotificationsDoNotDisturbFrom(SettingsActivity.this,
          ...
          if (!String.valueOf(millis)
              .equals(PreferenceUtils.getNotificationsDoNotDisturbTo(SettingsActivity.this))) {
            PreferenceUtils.setNotificationsDoNotDisturbTo(SettingsActivity.this,
                String.valueOf(millis));
            saveDoNotDisturb(true);
          }
  ...
  private void checkDoNotDisturb(boolean doNotDisturb) {
    ...
    if (doNotDisturbFrom.length() > 0) {
      mTextViewNotificationsDoNotDisturbFrom.setText(
          DateUtils.formatTimeWithMarker(Long.valueOf(doNotDisturbFrom)));
    } else {
      mTextViewNotificationsDoNotDisturbFrom.setText("");
    }
    ...
    if (doNotDisturbTo.length() > 0) {
      mTextViewNotificationsDoNotDisturbTo.setText(
          DateUtils.formatTimeWithMarker(Long.valueOf(doNotDisturbTo)));
    } else {
      mTextViewNotificationsDoNotDisturbTo.setText("");
    }
  }
  ...
  private void showSetProfileOptionsDialog() {
    ...
    builder.setTitle("Set profile image").setItems(options, new DialogInterface.OnClickListener() {
  ...
  private Uri getTempFileUri(boolean doNotUseFileProvider) {
    ...
    try {
      ...
      if (Build.VERSION.SDK_INT >= 24 && !doNotUseFileProvider) {
        uri = FileProvider.getUriForFile(this, "com.truethat.android.provider", tempFile);
      } else {
    ...
    SendBird.updateCurrentUserInfoWithProfileImage(nickname, profileImage,
        new SendBird.UserInfoUpdateHandler() {
          @Override public void onUpdated(SendBirdException e) {
            if (e != null) {
              // Error!
              Toast.makeText(SettingsActivity.this, "" + e.getCode() + ":" + e.getMessage(),
            ...
            try {
              PreferenceUtils.setProfileUrl(SettingsActivity.this,
                  SendBird.getCurrentUser().getProfileUrl());
              ImageUtils.displayRoundImageFromUrlWithoutCache(SettingsActivity.this,
                  Uri.fromFile(profileImage).toString(), imageView);
            } catch (Exception e1) {
              e1.printStackTrace();
            }
          }
