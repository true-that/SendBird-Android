...
public class WebUtils {
  ...
  public static abstract class UrlPreviewAsyncTask extends AsyncTask<String, Void, UrlPreviewInfo> {
    ...
    @Override protected UrlPreviewInfo doInBackground(String... params) {
      ...
      try {
        ...
        if (result.keySet().size() == 5) {
          return new UrlPreviewInfo(result.get("url"), result.get("site_name"), result.get("title"),
              result.get("description"), result.get("image"));
        }
      } catch (IOException e) {
        e.printStackTrace();
      }
    ...
    @Override protected abstract void onPostExecute(UrlPreviewInfo info);
  }
}
