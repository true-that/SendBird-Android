...
public class OpenChatFragment extends Fragment {
      ...
      case PERMISSION_WRITE_EXTERNAL_STORAGE:
        ...
        if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
          ...
          Snackbar.make(mRootLayout,
              "Storage permissions granted. You can now upload or download files.",
              Snackbar.LENGTH_LONG).show();
        } else {
          // Permission denied.
          Snackbar.make(mRootLayout, "Permissions denied.", Snackbar.LENGTH_SHORT).show();
        }
        break;
      default:
  ...
  @Override public void onAttach(Context context) {
    ...
    ((OpenChannelActivity) context).setOnBackPressedListener(
        new OpenChannelActivity.onBackPressedListener() {
  ...
  private void setUpChatAdapter() {
    ...
    mChatAdapter.setOnItemClickListener(new OpenChatAdapter.OnItemClickListener() {
      ...
      @Override public void onAdminMessageItemClick(AdminMessage message) {
      }
    });

    mChatAdapter.setOnItemLongClickListener(new OpenChatAdapter.OnItemLongClickListener() {
      @Override public void onBaseMessageLongClick(final BaseMessage message) {
  ...
  private void showUploadConfirmDialog(final Uri uri) {
    new AlertDialog.Builder(getActivity()).setMessage("Upload file?")
  ...
  private void enterChannel(String channelUrl) {
    OpenChannel.getChannel(channelUrl, new OpenChannel.OpenChannelGetHandler() {
      @Override public void onResult(final OpenChannel openChannel, SendBirdException e) {
        if (e != null) {
          // Error!
          e.printStackTrace();
          return;
   ...
   * <p>
   * A PreviousMessageListQuery must have been already initialized through {@link
   * #loadInitialMessageList(int)}
   */
