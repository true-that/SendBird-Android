package com.truethat.android.openchannel;
  ...
  @Override public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
    ...
    switch (holder.getItemViewType()) {
      ...
      case VIEW_TYPE_ADMIN_MESSAGE:
        ((AdminMessageHolder) holder).bind((AdminMessage) message, isNewDay, mItemClickListener);
        break;
      ...
      if (message.getType().toLowerCase().startsWith("image")) {
        ...
        if (thumbnails.size() > 0) {
          if (message.getType().toLowerCase().contains("gif")) {
            ImageUtils.displayGifImageFromUrl(context, message.getUrl(), fileThumbnail,
                thumbnails.get(0).getUrl(), fileThumbnail.getDrawable());
          } else {
            ImageUtils.displayImageFromUrl(context, thumbnails.get(0).getUrl(), fileThumbnail,
                fileThumbnail.getDrawable());
          }
        } else {
          if (message.getType().toLowerCase().contains("gif")) {
            ImageUtils.displayGifImageFromUrl(context, message.getUrl(), fileThumbnail,
                (String) null, fileThumbnail.getDrawable());
          } else {
        ...
        // If thumbnails exist, get smallest (first) thumbnail and display it in the message
        if (thumbnails.size() > 0) {
          ImageUtils.displayImageFromUrlWithPlaceHolder(context, thumbnails.get(0).getUrl(),
              fileThumbnail, R.drawable.ic_file_message);
        } else {
          fileThumbnail.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_play));
        }
      } else {
        fileThumbnail.setImageDrawable(
            context.getResources().getDrawable(R.drawable.ic_file_message));
      }
