...
import com.truethat.android.utils.TypingIndicator;
import java.io.File;
...
class GroupChannelListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  public void load() {
    try {
      ...
      File dataFile = new File(appDir,
          TextUtils.generateMD5(SendBird.getCurrentUser().getUserId() + "channel_list") + ".data");
    ...
    try {
      ...
      if (mChannelList != null && mChannelList.size() > 0) {
        ...
        File hashFile = new File(appDir,
            TextUtils.generateMD5(SendBird.getCurrentUser().getUserId() + "channel_list")
                + ".hash");
        File dataFile = new File(appDir,
            TextUtils.generateMD5(SendBird.getCurrentUser().getUserId() + "channel_list")
                + ".data");
  ...
  @Override public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
    ((ChannelHolder) holder).bind(mContext, mChannelList.get(position), mItemClickListener,
  ...
  private class ChannelHolder extends RecyclerView.ViewHolder {
    ...
    ChannelHolder(View itemView) {
      ...
      typingIndicatorContainer =
          (LinearLayout) itemView.findViewById(R.id.container_group_channel_list_typing_indicator);
    }
    ...
    void bind(final Context context, final GroupChannel channel,
      ...
      if (lastMessage != null) {
        ...
        if (lastMessage instanceof UserMessage) {
          ...
          String lastMessageString =
              String.format(context.getString(R.string.group_channel_list_file_message_text),
      ...
      indicatorImages.add(
          (ImageView) typingIndicatorContainer.findViewById(R.id.typing_indicator_dot_1));
      indicatorImages.add(
          (ImageView) typingIndicatorContainer.findViewById(R.id.typing_indicator_dot_2));
      indicatorImages.add(
          (ImageView) typingIndicatorContainer.findViewById(R.id.typing_indicator_dot_3));
    ...
    private void setChannelImage(Context context, GroupChannel channel,
        ...
        if (size >= 1) {
          ...
          if (!mChannelImageNumMap.containsKey(channel.getUrl())) {
            ...
            for (int index = 0; index < imageNum; index++) {
                ...
                @Override public void onResourceReady(Bitmap resource,
                  ...
                  if (index != null) {
                    ...
                    if (num != null) {
                      if (array.size() == num) {
                        MultiImageView multiImageView =
                            (MultiImageView) mChannelImageViewMap.get(channel.getUrl());
              ...
              RequestOptions myOptions = new RequestOptions().dontAnimate()
